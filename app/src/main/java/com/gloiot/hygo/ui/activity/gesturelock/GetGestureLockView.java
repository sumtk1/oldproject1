 ...
 * 
 */
public class GetGestureLockView extends RelativeLayout {

	private static final String TAG = "GetGestureLockView";
	...
	public GetGestureLockView(Context context, AttributeSet attrs)
	{
		this(context, attrs, 0);
	}

	public GetGestureLockView(Context context, AttributeSet attrs, int defStyle) {
...
//			mMarginBetweenLockView = (int) (mGestureLockViewWidth * 0.25);
	...
	public boolean onTouchEvent(MotionEvent event)
	{
		...
		case MotionEvent.ACTION_UP:
			...
			if (mOnGestureLockViewListener != null && mChoose.size() > 0) {
				...
				mOnGestureLockViewListener.onGettureLockResult(checkAnswer());
				if (!checkAnswer()) {
					if (this.mTryTimes == 0) {
						mOnGestureLockViewListener.onUnmatchedExceedBoundary();
					} else {
						mOnGestureLockViewListener.onUnmatched(mTryTimes);
					}
				}
			}
	...
	private boolean checkAnswer() {
		...
		String passwd = SharedPreferencesUtils.getInstance().getSharedPreferences().getString(ConstantUtlis.SP_GESTURELOCK_RESULT, "");

		if (passwd.length() != mChoose.size())
			return false;

		String choose = "";
		for (int i = 0; i < mChoose.size(); i++) {
			choose = choose + mChoose.get(i);
		}

		Log.e("--checkAnswer--", passwd + "=====" + choose);

		if (choose.equals(passwd)) {
			return true;
		}
