...
public class WebToPayManager implements NetworkListener {
    ...
    public static Intent toCashier(SharedPreferences preferences, Context mContext, JSONObject jsonObject) {
        // 拼接收银台地址
        String url = preferences.getString(ConstantUtlis.WEBCASHIER_URL, "") +
    ...
    private void showPwdBox(final int tag, final String toPayUrl, final JSONObject jsonObject, String money, final String randomCode) {
        DialogUtlis.customPwd(((BaseActivity) mContext).getmDialog(), money, true, new MDialogInterface.PasswordInter() {
            @Override
    ...
    private void pay2(String paytype, JSONObject response) {
        switch (paytype) {
            case "wx":
                ...
                wxPay.startPay(mContext, response, ConstantUtlis.WX_APP_ID2, new WXPay.WXPayCallBack() {
                    @Override
            ...
            case "bank":
                ...
                unionPay.startPay(mContext, JSONUtlis.getString(response, "tn_code", "123"), ConstantUtlis.UnionPayMode, new UnionPay.UnionPayCallBack() {
                    @Override
                    public void paySuccess() {
                        fragment.stopTimer();
                        webManage.callBack(TAG_WEBMANAGE_SUCCEESS);
