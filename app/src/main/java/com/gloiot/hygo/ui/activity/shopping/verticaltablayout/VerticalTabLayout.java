...
import com.gloiot.hygo.ui.activity.shopping.verticaltablayout.widget.TabIndicator;
...
//            if (mViewPager == null)
    ...
    public void setTabAdapter(TabAdapter adapter, int defaultPosition) {
        setTabAdapter(adapter);
        mTabStrip.setDefaultPosition(defaultPosition);
        TabView firstView = getTabAt(0);
        if (firstView != null) {
            firstView.setChecked(false);
        }
        TabView defaultView = getTabAt(defaultPosition);
        if (defaultView != null) {
            defaultView.setChecked(true);
            mSelectedTab = defaultView;
        }
    }
...
//        if (mViewPager != null) return mViewPager.getCurrentItem();
    ...
    private class TabStrip extends LinearLayout {
        ...
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            Log.e("onMeasure", "测量过程");
            if (getChildCount() > 0) {
                View childView = getChildAt(0);
                mIndicatorHeight = childView.getMeasuredHeight();
                if (mInvalidateCount == 0) {
//                    mIndicatorBottomY = mIndicatorHeight;
                    mIndicatorY = calcIndicatorY(defaultPosition);
                    mIndicatorBottomY = mIndicatorY + mIndicatorHeight;
                    Log.e("mIndicatorY", "mIndicatorY = " + mIndicatorY + "   defaultPosition = " + defaultPosition);
