...
public class ShouHouFailActivity extends BaseActivity implements View.OnClickListener {
    //商品缩略图
    ...
    //拒绝原因
    @Bind(R.id.tv_shouhou_fail_jujueyuanyin)
    TextView tvShouhouFailJujueyuanyin;
    //售后信息头部说明
    ...
    public int initResource() {
        return R.layout.activity_shouhou_fail;
    }

    @Override
    public void initData() {
        CommonUtlis.setTitleBar(this, "申请失败");

        myDingdanId = getIntent().getStringExtra("订单id");
        myShangpinId = getIntent().getStringExtra("商品id");
        myId = getIntent().getStringExtra("id");
        mType = getIntent().getStringExtra("申请售后");
        dingdanzhuangtai = getIntent().getStringExtra("订单状态");

        initComponent();

        if ("退款".equals(mType)) {
            requestHandleArrayList.add(requestAction.shop_rf_result_fail(this, myDingdanId, myShangpinId, myId, "0"));
            layoutTvKuituanxinxi.setText("退款信息");
        } else {
            requestHandleArrayList.add(requestAction.shop_rf_result_fail(this, myDingdanId, myShangpinId, myId, "1"));
    ...
    public void requestSuccess(int requestTag, JSONObject response, int showLoad) throws JSONException {
        ...
        switch (requestTag) {
            case RequestAction.TAG_RFRESULTFAIL:
                ...
                layoutTvTuikuanXiangqingTongyishijian1.setText("处理时间：");
                layoutTvTuikuanXiangqingTongyishijian.setText(JSONUtlis.getString(response, "处理时间"));
                layoutTvTuikuanXiangqingBianhao.setText(JSONUtlis.getString(response, "订单id"));

                tvShouhouFailJujueyuanyin.setText(JSONUtlis.getString(response, "拒绝原因"));
    ...
    @OnClick({
            R.id.btn_shouhou_fail_kefujieru, //客服介入
            R.id.btn_shouhou_fail_zaicishenqing //再次申请
    })
    @Override
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.btn_shouhou_fail_kefujieru:
                Intent it = new Intent(mContext, SelectKeFuActivity.class);
                it.putExtra("客服组名称", "商城客服");
                startActivity(it);
                break;

            case R.id.btn_shouhou_fail_zaicishenqing:
